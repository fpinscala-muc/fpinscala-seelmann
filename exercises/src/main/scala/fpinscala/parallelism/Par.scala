...
object Par {
  ...
  // It doesn't use the `ExecutorService` at all. It's always done and can't be 
  ...
  // controlling parallelism. We can always do `fork(map2(a,b)(f))` if we want 
  ...
  def map2[A,B,C](a: Par[A], b: Par[B])(f: (A,B) => C): Par[C] = 
    (es: ExecutorService) => {
      ...
      // and wraps them in a `UnitFuture`. In order to respect timeouts, we'd need 
      // a new `Future` implementation that records the amount of time spent evaluating `af`, 
  ...
  // some problems with it--for one, the outer `Callable` will block waiting 
  // for the "inner" task to complete. Since this blocking occupies a thread in our 
  // thread pool, or whatever resource backs the `ExecutorService`, this implies that 
  // we're losing out on some potential parallelism. Essentially, we're using two 
